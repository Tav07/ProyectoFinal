...
def registrar_entrada():
    ...
    if placa == "":
        entrada_label.config(text="Error de registro, placa vacía")
        return
...
def mostrar_espacios_disponibles():
    espacios_totales = 100  # número total de espacios
    espacios_ocupados = len(vehiculos_registrados)
    espacios_libres = espacios_totales - espacios_ocupados

    mensaje = f"Hay {espacios_libres} espacios disponibles de {espacios_totales}"
...
factura_text.configure(state='disabled')
mostrar_espacios_btn = tk.Button(window, text="Mostrar espacios disponibles", command=mostrar_espacios_disponibles)
...
factura_text.pack()
mostrar_espacios_btn.pack()
estado_label = tk.Label(window, text="")
estado_label.pack()
